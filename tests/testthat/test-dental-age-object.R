...
test_that("the samplers survive an estimate with no finite value", {
  # Early teeth only at Ac, no M2 or M3 to derive from: still no estimate.
  x <- data.frame(
    Sex = "F",
    Canine = "Ac",
    P3 = "Ac",
...
test_that("estimate_age_hdi carries the threshold through", {
  ...
  expect_named(
    hdi,
    c("lower_bound", "upper_bound", "completion_threshold")
  )
  expect_equal(
    as.numeric(hdi["completion_threshold"]),
