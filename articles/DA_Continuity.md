...
- Both sexes introduce teeth in the same order: P3 at row 3, M1 at row 6
  (already at `Cr.c` because it develops earliest), M2 at row 8, P4 at
  row 9.
- The male progression uses stages that lag behind the female for the
  canine (~1 stage) and P3 (~1 stage), reflecting that males develop
  these teeth later. M1 and P4 are minimally offset. M3 is introduced
  one row earlier for males (row 19 vs 20), reflecting slightly earlier
  M3 development in males.
- For females, M3 first appears at row 20; for males, at row 19. For
  females, late M3 stages (`R.5` through `A.5`) have no published
  log-normal parameters and are excluded from the age estimate. For
  males, all M3 stages have parameters.
- M1 reaches `A.c` at row 19 for females and row 20 for males, shifting
  from the age estimate to the completion threshold. The `compatibility`
  column shows whether the two agree.
